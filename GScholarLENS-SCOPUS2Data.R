...
get_complete_scopus_data <- function(api_key = NULL, orcid, rv, output, session = NULL) {
  ...
  # --- Build Base Request using httr2 (Local R only) ---
  if (!is_WASM) {
  ...
  repeat {
    ...
    if (is_WASM) {
      # =========================================================================
      # BRANCH 1: WebR / WASM (Using patched base url() and manual encoding)
      # =========================================================================
      ...
      res_err <- tryCatch({
        ...
        conditionMessage(e) # Returns the error string
      }, finally = {
        # CRITICAL: Prevent WASM 'index out of bounds' connection limit crashes
        if (!is.null(con) && inherits(con, "connection")) try(close(con), silent = TRUE)
      })
      
      # If an error occurred, try to deduce the status code from the R error string
      if (!is.null(res_err)) {
        if (grepl("401", res_err)) {
          status <- 401
        } else if (grepl("429", res_err)) {
          status <- 429
        } else {
          status <- 500
        }
      ...
      if (status == 200) {
        ...
        err_body <- resp_body_string(resp_pubs)
      }
    }
    
    # --- SMART ERROR HANDLING (Works for both environments) ---
    if (status == 401) {
      network_issue <- detect_vpn(rv, output)
      
      if (!is.null(network_issue)) {
        stop_msg <- sprintf("Authorization Error (401): %s.", network_issue)
        stop(stop_msg)
      } else {
        rv$log_text <- paste(
          rv$log_text,
      ...
      rv$log_text <- paste(
        rv$log_text,
        paste("API Error:", status, err_body),
        sep = "\n"
      )
      output$log <- renderText({ rv$log_text })
      stop(paste("API Error:", status, err_body))
    }
    # ----------------------------
